...
def main():
    ...
    run_dir = os.path.join(log_dir, dataset, f"sensitivity-{datetime.now().strftime('%Y%m%d-%H%M%S')}")
    ...
    for top_n in top_ns:
        for k_val in k_vals:
            logger.info(f"Running top_n={top_n}, k={k_val}")
            # fresh model
            ...
            induce_concepts(H, assign, top_n, llm, texts=texts, run_dir=run_dir, tracker=tracker)
            ...
            gen_map = synthesize_for_S(S_set, neighbors, texts, k_val, llm, run_dir=run_dir, tracker=tracker)
            ...
            results[f"top{top_n}_k{k_val}"] = metrics
            logger.info(f"top_n={top_n}, k={k_val}, metrics={metrics}")

    with open(os.path.join(run_dir, "sensitivity.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    tracker.save(os.path.join(run_dir, "costs.json"))

    # simple plot for ACC across k for each top_n
    plt.figure()
    for top_n in top_ns:
        accs = [results[f"top{top_n}_k{k}"]["ACC"] for k in k_vals]
        plt.plot(k_vals, accs, marker="o", label=f"top_n={top_n}")
    plt.xlabel("k_neighbors")
    plt.ylabel("ACC")
    plt.legend()
    plt.title("Sensitivity ACC")
    plt.tight_layout()
    plt.savefig(os.path.join(run_dir, "sensitivity.png"))

    logger.info(f"Sensitivity results saved to {run_dir}")
